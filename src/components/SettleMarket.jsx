...
const GetMyMarkets = () => {
  ...
  const handleConfirmWinner = async (winner) => {
    console.log(winner);
    try {
      console.log('Winner selected:', winner); // Debugging line to check if button click is captured
      console.log('Selected Market ID:', selectedMarket.id); // Log selected market ID to verify the market is selected
      // Call API to settle the market and set the outcome
      const response = await settleMarket(selectedMarket.id, winner);
      
      console.log('API Response:', response); // Log the API response to see what comes back
      ...
      if (response.data.success) {
        ...
        console.error('Failed to settle market: ', response); // Log the error if the response is not successful
        setMessage('Failed to settle market');
      }
    } catch (error) {
      console.error('Error settling market:', error); // Log any error during API call
